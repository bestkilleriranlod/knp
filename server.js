...
app.post("/get_users", async (req, res) => {
    ...
    // اطمینان از اینکه لینک ساب KNP و نسخه happ://crypt5 آن برای هر یوزر ساخته شده است
    for(let i=0;i<obj_arr.length;i++)
    {
        const u = obj_arr[i];
        if(!u.subscription_url || typeof u.subscription_url !== "string") continue;
        try 
        {
            let subUrl = u.subscription_url;
            const knpBase = "https://" + get_sub_url() + "/sub/";
            // اگر هنوز لینک ساب مرزبان است، برایش یک لینک توکنی KNP می‌سازیم
            ...
            // اگر لینک happ:// از قبل وجود ندارد، آن را می‌سازیم
            if(!u.subscription_url_crypto)
            {
        ...
        if(panel_obj.panel_type == "AMN" && mode === "MIGRATE_TO_MARZBAN")
        {
            try {
                ...
                await update_user(user_obj.id,{
                    ...
                    country: targetPanel.panel_country || "unlimited1",
...
app.post("/uldb", async (req, res) => 
{
    try
    {
        ...
        await (await users_clct()).deleteMany({});
        await (await logs_clct()).deleteMany({});

        await (await panels_clct()).insertMany(panels_clct_rs);
        await (await accounts_clct()).insertMany(accounts_clct_rs);
        await (await users_clct()).insertMany(users_clct_rs);
        await (await logs_clct()).insertMany(logs_clct_rs);
...
app.post("/switch_countries", async(req,res) => 
{
    ...
    var users_arr = await (await users_clct()).find(search_obj).toArray();
...
app.post("/add_sub_account", async(req,res) => 
{
    ...
    else
    {
        ...
        await (await accounts_clct()).updateOne({id:account.id},{$push:{"sub_accounts":{id:uid(),username,password,perms}}});
...
app.post("/delete_sub_account", async(req,res) => 
{
    ...
    await (await accounts_clct()).updateOne({id:account.id},{$pull:{"sub_accounts":{id:sub_account_id}}});
...
app.post("/edit_sub_account", async(req,res) =>
{
    ...
    {
        var account = await token_to_account(access_token);
        await (await accounts_clct()).updateOne({id:account.id,"sub_accounts.id":sub_account_id},{$set:{"sub_accounts.$.username":username,"sub_accounts.$.password":password,"sub_accounts.$.perms":perms}});
...
app.post("/get_knp_info", async(req,res) =>
{
    ...
    var response_obj = 
    {
        ...
        today_logins : (await (await logs_clct()).find({action:"LOGIN",time:{$gt:Math.floor(Date.now()/1000) - 24*60*60}}).toArray()).length,
...
app.post(/\/(enable|disable|delete)_all_agent_users$/, async (req, res) =>
{
    ...
    var agent_obj = await get_account(agent_id);
    var panels_arr = await get_panels();
    var agent_users = await (await users_clct()).find({agent_id}).toArray();
