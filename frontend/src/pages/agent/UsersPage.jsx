...
const UsersPage = () => {
    ...
    const handleEditUser = async (user_id, data_limit, expire, country, protocols, flow_status, desc, safu, is_reset_data=false, mode="renewal") => {
        setEditMode(true)
        const access_token = sessionStorage.getItem("access_token")
        var req_res = await axios.post("/edit_user", { access_token, user_id, data_limit, expire, country, protocols, flow_status, desc, safu, is_reset_data, mode })
        if (req_res.data.status === "ERR") {
            setError_msg(req_res.data.msg)
            setHasError(true)
            setEditMode(false)
